...
class AddFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.options = [
            str(i) + " Threads" for i in range(1, multiprocessing.cpu_count() + 1)
        ]
        ...
        self.setButton = tk.Button(
            ...
            command=lambda: threading.Thread(target=self.set_location).start(),
        )
        self.downloadButton = tk.Button(
            self,
            text="Download",
            height=2,
            command=lambda: threading.Thread(target=self.download).start(),
        )
        ...
        if self.prev_items != self.items or self.status != self.prev_status:
            ...
            for i, item in enumerate(reversed(self.items)):
                ...
                tk.Button(
                    self,
                    text="📁",
                    command=lambda i=item.file_path: subprocess.Popen(
                        r'explorer /select,"{path}"'.format(path=i)
                    ),
